...
def run_pipeline(
    ...
    try:
        ...
        if "out of memory" in str(e).lower():
            console.print("[bold red]CUDA out of memory while loading the model![/bold red]")
...
def build_parser() -> argparse.ArgumentParser:
    """Builds the legacy pseudoMSA argument parser."""
    ...
    parser.add_argument(
        ...
        help=f"List of coverage values. Default: {DEFAULT_COVERAGE_VALUES}",
    )
    ...
    parser.add_argument(
        "--mutation_rates",
        type=str,
        default=DEFAULT_MUTATION_RATES_STR,
        help=f"JSON string for mutation rates. Default: '{DEFAULT_MUTATION_RATES_STR}'",
    )
    parser.add_argument(
        "--sample_percentage",
        type=float,
        default=1.0,
        help="Percentage of sequences to sample for evolution (e.g., 0.5 for 50%%).",
    )
    return parser


def main(argv: Optional[List[str]] = None) -> None:
    """CLI entrypoint retained for script compatibility."""
    parser = build_parser()
    args = parser.parse_args(argv)
    from ghostfold.config import MSAWorkflowConfig
    from ghostfold.errors import GhostfoldError
    from ghostfold.services import run_msa_workflow
