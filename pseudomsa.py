...
def run_pipeline(
    ...
    try:
        ...
        tokenizer = T5Tokenizer.from_pretrained(MODEL_NAME, use_safetensors=True, low_cpu_mem_usage=True, do_lower_case=False, legacy=True)
        model = AutoModelForSeq2SeqLM.from_pretrained(MODEL_NAME, use_safetensors=True, low_cpu_mem_usage=True).to(device)
        model.eval()
        if device.type == 'cuda':
            model.half()
